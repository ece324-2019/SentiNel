...
dataset1 = pd.read_csv('./data/sentiment140.csv', delimiter=',',encoding='latin-1')
# dataset2 = pd.read_csv('./data/sentimenttreebank.csv', delimiter=',')
# dataset3 = pd.concat([dataset1,dataset2])
print((dataset1))

remain, train_data = train_test_split(dataset1, test_size = 3000, random_state = 1, stratify = dataset1['label'])
valid_data, test_data = train_test_split(remain, test_size = 1000, random_state = 1,stratify = remain['label'])
